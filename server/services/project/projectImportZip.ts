...
const log = createScopedLogger("ProjectImportZip");

export async function importProjectZip(
  uploadedFilePath: string,
  universeResolutions?: Record<string, "keep_local" | "overwrite" | "ingest_as_new">
): Promise<string> {
  log.info(`Starting ZIP import from: ${uploadedFilePath}`);
  ...
  if (nonHiddenFiles.length > 0) {
    ...
    if (parts.length > 1) {
      ...
      if (allSharePrefix) {
        ...
        log.info(`Detected top-level folder wrapper prefix in ZIP: "${commonPrefix}"`);
      }
    }
  }
    ...
    if (normPath === "assets_db.json" || fname === "assets_db.json") {
      try {
        ...
        if (Array.isArray(parsed)) {
          assetsDbMeta = parsed;
          log.info(`Found assets_db.json in ZIP with ${assetsDbMeta.length} assets`);
        }
      } catch (e) {
        log.error("Failed to parse assets_db.json", { error: e });
      }
      ...
      try {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          log.info(`Found universe/characters.json in ZIP with ${Object.keys(parsed).length} characters`);
        }
      } catch (e) {
        log.error("Failed to parse universe/characters.json", { error: e });
      }
      ...
      if (!IGNORED_JSON_FILENAMES.has(fname.toLowerCase())) {
        try {
          ...
          if (parsed && typeof parsed === "object" && (parsed.scene_name || Array.isArray(parsed.shots) || parsed.scene_planning)) {
            ...
            log.info(`Found master project scene JSON file in ZIP: "${normPath}"`);
          } else {
            log.info(`Skipping non-project JSON at root: "${normPath}"`);
          }
        } catch (e) {
          log.error(`Error parsing candidate JSON file "${normPath}"`, { error: e });
        }
      }
    }
  }
        ...
        if (!IGNORED_JSON_FILENAMES.has(fname.toLowerCase())) {
          try {
            ...
            if (parsed && typeof parsed === "object") {
              importedProject = fname.replace(/\.json$/i, "");
              projectJsonData = parsed;
              log.info(`Fallback: Accepted master project JSON from: "${normPath}"`);
              break;
            }
          } catch (e) {}
        }
  ...
  if (!projectJsonData) {
    log.warn("No valid project scene JSON found inside the ZIP archive!");
  }

  // If resolutions are provided, apply them
  if (universeResolutions && Object.keys(universeResolutions).length > 0) {
    log.info("Applying character import resolutions...");
    ...
    log.info("Auto-ingesting characters...");
  ...
  log.info(`Resolving scene name: "${rawSceneName}" -> directory: "${sceneDirName}"`);
  ...
  for (const file of directory.files) {
    ...
    if (normPath.startsWith("workflows/") || normPath.startsWith("staged_workflows/")) {
      ...
      log.info(`Extracted workflow: "${normPath}" -> "${destPath}"`);
      ...
      if (isUniverse) {
        ...
        log.info(`Extracted global universe asset: "${normPath}" -> "${destPath}"`);
      } else {
        ...
        log.info(`Extracted upload: "${normPath}" -> "${destPath}"`);
      }
      ...
      fs.writeFileSync(destPath, buffer);
      log.info(`Extracted take/render video: "${normPath}" -> "${destPath}"`);
      ...
      if (!IGNORED_JSON_FILENAMES.has(fname.toLowerCase())) {
        ...
        log.info(`Extracted project scene file to base: "${destBase}"`);
      }
    }
  }
  ...
  if (!fs.existsSync(authoritativeJson) && projectJsonData) {
    ...
    log.info(`Assured authoritative scene JSON file is written to: "${authoritativeJson}"`);
  }
    ...
    try {
      ...
      if (fs.existsSync(ASSET_DB_FILE)) {
        try {
          ...
          log.error("Error reading ASSET_DB_FILE", { error: e });
        }
      }
      ...
      log.info(`Merged and saved ${registeredAssets.length} asset metadata records into "${ASSET_DB_FILE}"`);
    } catch (e) {
      log.error("Error merging asset records in ASSET_DB_FILE", { error: e });
    }
  }

  try {
    fs.unlinkSync(uploadedFilePath);
    log.info(`Cleaned up temporary upload file at: ${uploadedFilePath}`);
  } catch (e) {}

  log.info(`Successfully completed ZIP import. Authoritative file: "${sceneDirName}.json"`);
