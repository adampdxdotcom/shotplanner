  ...
  return (
    <div className={`character-asset-card bg-white dark:bg-zinc-900/80 p-3 rounded-xl border-2 border-zinc-200 dark:border-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-700 transition-all space-y-2 relative group flex flex-col shadow-xs hover:shadow-sm ${className}`}>
      ...
      {isImage ? (
        ...
        <div className="relative w-full aspect-square bg-indigo-50/40 dark:bg-zinc-950 rounded-lg overflow-hidden border border-indigo-100 dark:border-zinc-800 flex flex-col items-center justify-center text-indigo-600 dark:text-indigo-400 gap-2">
        ...
        <div className="relative w-full aspect-square bg-emerald-50/40 dark:bg-zinc-950 rounded-lg overflow-hidden border border-emerald-100 dark:border-zinc-800 flex flex-col items-center justify-center text-emerald-600 dark:text-emerald-400 gap-2">
      ...
      {asset.description && (
        <p className="text-[11px] text-zinc-600 dark:text-zinc-300 line-clamp-2 italic bg-zinc-50 dark:bg-zinc-950/60 p-1.5 rounded border border-zinc-200 dark:border-zinc-800">
          "{asset.description}"
        </p>
      )}

      <div className="flex items-center justify-between text-[10px] text-zinc-500 dark:text-zinc-400 pt-1 border-t border-zinc-100 dark:border-zinc-800 mt-auto">
...
export const EmptySlotCard: React.FC<{ idx: number, type: string, className?: string, onClick: () => void }> = ({ idx, type, className = "", onClick }) => (
  ...
  >
    ...
    <span className="text-xs font-semibold mb-1 uppercase tracking-wider text-zinc-700 dark:text-zinc-300 group-hover:text-indigo-700 dark:group-hover:text-indigo-300 transition-colors">
      Upload Slot
    </span>
    <span className="font-mono text-[10px] font-medium text-zinc-500 dark:text-zinc-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
