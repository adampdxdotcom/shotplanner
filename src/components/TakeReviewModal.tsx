      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-5xl flex flex-col max-h-full overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between p-4 border-b border-zinc-800 bg-zinc-900/50">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {matchedVar && (
              <span className="flex items-center gap-1 text-xs font-mono font-semibold px-2.5 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-lg">
                <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                {matchedVar.label || `Variation ${matchedVar.variation_number}`}
              </span>
            )}
            {take.is_hero && (
