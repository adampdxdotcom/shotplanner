...
import { Eraser, Paintbrush, Feather, RotateCcw, Check, Undo2, Redo2 } from "lucide-react";
...
export interface CanvasMaskingHudProps {
  ...
  setMaskMode: (mode: "erase" | "restore" | "refine") => void;
  brushSize: number;
...
export const CanvasMaskingHud: React.FC<CanvasMaskingHudProps> = ({
  ...
  return (
    <div
      ...
      className="masking-hud-floating absolute top-3 left-1/2 -translate-x-1/2 z-[100] bg-zinc-950/95 border border-indigo-500/70 rounded-xl px-3 py-2 shadow-2xl backdrop-blur flex items-center gap-3 text-xs max-w-[95%] sm:max-w-none flex-wrap sm:flex-nowrap justify-center animate-in fade-in zoom-in-95 duration-150"
    >
      {/* Actor Header */}
      <div className="flex items-center gap-1.5 border-r border-zinc-800 pr-2.5">
        ...
        <div>
          ...
          <div className="text-[9px] text-zinc-400 leading-tight">
            {maskMode === "erase"
              ? "Erase pixels to tuck behind foreground objects"
              : maskMode === "refine"
              ? "Refine Edge: Recover fine hair strands while despilling green/blue"
              : "Paint over erased areas to restore"}
          </div>
        </div>
      </div>

      {/* Erase vs Restore vs Refine Edge Toggle */}
      <div className="flex items-center bg-zinc-900 border border-zinc-800 rounded-lg p-0.5">
        <button
          ...
          className={`px-2 py-1 rounded text-[11px] font-medium flex items-center gap-1 transition-all cursor-pointer ${
            maskMode === "erase"
              ? "bg-red-950/90 text-red-300 border border-red-600/80 shadow-sm font-semibold"
        ...
        <button
          ...
          className={`px-2 py-1 rounded text-[11px] font-medium flex items-center gap-1 transition-all cursor-pointer ${
            maskMode === "restore"
              ? "bg-emerald-950/90 text-emerald-300 border border-emerald-600/80 shadow-sm font-semibold"
              : "text-zinc-400 hover:text-white"
          }`}
        ...
        <button
          type="button"
          onClick={() => setMaskMode("refine")}
          className={`px-2 py-1 rounded text-[11px] font-medium flex items-center gap-1 transition-all cursor-pointer ${
            maskMode === "refine"
              ? "bg-amber-950/90 text-amber-300 border border-amber-500/80 shadow-sm font-bold"
              : "text-zinc-400 hover:text-amber-300"
          }`}
          title="Fine Edge Refinement Brush: Brush back wispy hair strands and transparent fabrics without bringing green screen back"
        >
          <Feather className="w-3 h-3 text-amber-400" />
          <span>Refine Hair</span>
        </button>
      </div>
      ...
      {/* Action Buttons: Undo / Redo & Reset & Done */}
      <div className="flex items-center gap-1.5">
        <div className="flex items-center bg-zinc-900 border border-zinc-800 rounded-lg p-0.5 mr-1">
          <button
            ...
            title="Undo mask stroke (Ctrl+Z / ⌘Z)"
          >
          ...
          <button
            ...
            title="Redo mask stroke (Ctrl+Y / ⌘Shift+Z)"
          >
