  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-zinc-900/60 border border-zinc-800 p-3 rounded-xl">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full bg-zinc-950/80 border border-zinc-800 rounded-lg pl-9 pr-3 py-1.5 text-xs text-zinc-200 placeholder-zinc-500 outline-none focus:border-amber-500/50"
          />
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <div className="flex items-center bg-zinc-950 border border-zinc-800 rounded-lg p-0.5">
            <button
              ...
              className={`px-2.5 py-1 text-xs font-semibold rounded-md transition-colors ${
                filterType === "all" ? "bg-zinc-800 text-amber-300" : "text-zinc-400 hover:text-zinc-200"
              }`}
            >
              All ({charList.length})
            </button>
            <button
              ...
              className={`px-2.5 py-1 text-xs font-semibold rounded-md transition-colors flex items-center gap-1 ${
                filterType === "characters" ? "bg-zinc-800 text-indigo-300" : "text-zinc-400 hover:text-zinc-200"
              }`}
            >
            ...
            <button
              ...
              className={`px-2.5 py-1 text-xs font-semibold rounded-md transition-colors flex items-center gap-1 ${
                filterType === "locations" ? "bg-zinc-800 text-emerald-300" : "text-zinc-400 hover:text-zinc-200"
              }`}
            >
          ...
          <button
            ...
            className="bg-amber-600 hover:bg-amber-500 text-black px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5 shrink-0 shadow-sm"
          >
      ...
      {filteredCharacters.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-zinc-800 rounded-2xl bg-zinc-900/30">
          <div className="w-16 h-16 bg-zinc-800/60 rounded-full flex items-center justify-center mb-4 text-amber-400">
            <Globe className="w-8 h-8 opacity-70" />
          </div>
          <h3 className="text-lg font-bold text-zinc-200 mb-2">No Universe Entities Found</h3>
          ...
          <button
            type="button"
            onClick={onCreateNewUniverseChar}
            className="bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold px-4 py-2 rounded-lg text-xs transition-colors flex items-center gap-2"
          >
          ...
          {filteredCharacters.map(char => {
            ...
            return (
              <div 
                ...
                className="bg-zinc-900/60 border border-amber-900/30 hover:border-amber-500/40 rounded-2xl overflow-hidden flex flex-col md:flex-row shadow-xl transition-all"
              >
                {/* Profile Card Left Panel */}
                <div className="w-full md:w-80 lg:w-88 bg-zinc-900/90 p-6 border-b md:border-b-0 md:border-r border-zinc-800 flex flex-col shrink-0">
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-14 h-14 rounded-full bg-zinc-950 border-2 overflow-hidden shrink-0 flex items-center justify-center ${
                        isLoc ? "border-emerald-500/40 text-emerald-400" : "border-amber-500/50 text-amber-400"
                      }`}>
                        {profilePic ? (
                          <img 
                          ...
                          <span className="text-lg font-bold text-amber-300">{subject.charAt(0).toUpperCase()}</span>
                        )}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-base font-bold text-zinc-100 line-clamp-1" title={subject}>{subject}</h3>
                          <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-950/80 border border-amber-600/50 text-amber-300 flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${
                            isLoc ? "bg-emerald-950 text-emerald-300 border border-emerald-800/50" : "bg-indigo-950 text-indigo-300 border border-indigo-800/50"
                          }`}>
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className={`px-2.5 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors shrink-0 shadow-sm cursor-pointer ${
                          isAlreadyInScene
                      ...
                      <button
                        ...
                        className="p-1.5 text-zinc-500 hover:text-red-400 hover:bg-red-950/40 rounded-lg transition-colors shrink-0 cursor-pointer"
                  ...
                  <div className="flex-1 space-y-3">
                    <div>
                      <label className="text-[10px] uppercase font-bold text-zinc-400 tracking-wider mb-1 flex items-center justify-between">
                      ...
                      <textarea
                        ...
                        className="w-full bg-zinc-950/80 border border-zinc-800 rounded-lg p-2 text-xs text-zinc-300 resize-none outline-none focus:border-amber-500/50 min-h-[60px]"
                      />
                    </div>
                    ...
                    {!isLoc && (
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-zinc-950/80 border border-zinc-800 rounded-lg p-2 text-xs text-zinc-300 outline-none focus:border-amber-500/50"
                        />
                      </div>
                    )}
                  </div>
                  ...
                  {/* Action Bar */}
                  <div className="flex items-center gap-2 pt-4 mt-4 border-t border-zinc-800">
                    <button
                      type="button"
                      onClick={() => onOpenBulkUpload(subject, isLoc)}
                      className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 border border-zinc-700 px-2.5 py-1.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 bg-indigo-950/70 hover:bg-indigo-900/80 text-indigo-300 border border-indigo-800/60 px-2.5 py-1.5 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
                    >
                ...
                <div className="p-4 md:p-6 flex-1 bg-zinc-950/30 overflow-x-auto min-w-0">
                    ...
                    {charAssets.length === 0 ? (
                      <div className="flex items-center gap-3">
                        {[1, 2, 3].map(i => (
                          <div 
                            key={`placeholder-${i}`} 
                            className="w-32 h-40 border-2 border-dashed border-zinc-800/40 rounded-xl bg-zinc-900/20 shrink-0 flex flex-col items-center justify-center p-3 text-center"
                          >
                            <span className="text-[11px] text-zinc-600 font-medium">Slot {i}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
      ...
      {characterToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <h3 className="text-base font-bold text-zinc-100 mb-2">Remove from Universe Roster?</h3>
            <p className="text-xs text-zinc-400 mb-6 leading-relaxed">
              Are you sure you want to delete <span className="text-amber-300 font-semibold">"{characterToDelete}"</span> from the global universe roster? This will not delete scene copies already active in specific projects.
            </p>
