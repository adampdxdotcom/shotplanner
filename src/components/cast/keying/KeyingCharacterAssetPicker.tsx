...
export const KeyingCharacterAssetPicker: React.FC<KeyingCharacterAssetPickerProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
      ...
      <div className="md:col-span-4 bg-slate-50/70 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3.5 flex flex-col gap-3 shadow-2xs">
        <label className="text-xs font-bold text-zinc-800 dark:text-zinc-300 uppercase tracking-wider flex items-center gap-1.5">
        ...
        <select
          ...
          className="w-full bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded-lg px-2.5 py-2 text-xs text-amber-700 dark:text-amber-400 font-bold outline-none focus:border-amber-500 cursor-pointer shadow-2xs"
        >
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-lg p-3 text-center cursor-pointer transition-colors flex flex-col items-center justify-center gap-1.5 ${
            dragActive 
              ...
              : "border-zinc-300 hover:border-indigo-400 bg-white dark:border-zinc-800 dark:hover:border-zinc-700 dark:bg-zinc-950/60 text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-200 shadow-2xs"
          }`}
        >
          <input
            ref={fileInputRef}
      ...
      <div className="md:col-span-8 bg-slate-50/70 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3.5 flex flex-col gap-2.5 shadow-2xs">
        ...
        {characterAssets.length === 0 && !customImageSrc ? (
          <div className="h-36 flex flex-col items-center justify-center text-center p-4 border border-dashed border-zinc-300 dark:border-zinc-800 rounded-lg text-zinc-500 text-xs gap-2">
          ...
          <div className="flex items-center gap-2.5 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-zinc-300 dark:scrollbar-thumb-zinc-800">
            ...
            {customImageSrc && (
              <div
                ...
                className={`relative shrink-0 w-24 h-24 rounded-lg overflow-hidden border-2 cursor-pointer transition-all ${
                  !selectedAsset ? "border-amber-500 ring-2 ring-amber-500/40" : "border-zinc-300 dark:border-zinc-800 opacity-60"
                }`}
              >
                ...
                <div
                  ...
                  className={`relative shrink-0 w-24 h-24 rounded-lg overflow-hidden border-2 cursor-pointer transition-all group ${
                    isSelected
                      ...
                      : "border-zinc-200 dark:border-zinc-800 hover:border-zinc-400 dark:hover:border-zinc-700 opacity-80 hover:opacity-100"
                  }`}
                >
                  <img
